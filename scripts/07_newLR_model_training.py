...
Fit LSTM or BiLSTM model using new LR 
only n pixels where previous LR produced PCC in training + validation < 0.60
...
class hyperparameters():
    ...
    def load_optuna(self, optuna_dir, model_name, clima_name):
        ...
        LRstudy = optuna.load_study(study_name = clima_name + "-study_LR_OPTUNA_" + model_name, 
                                    storage = "sqlite:///" + optuna_dir + "newLR/" + clima_name + "_LR_OPTUNA_" + model_name + ".db")

        LRtrial = LRstudy.best_trial
    
        for key, value in LRtrial.params.items():
            if key == "learning_rate":
                self.newl_rate = value
...
def main():
    ...
    data_arr, input_cols, target_col = load_data(data_dir, clima_name, features, id_refitPIX)
    ...
    for p in range(pix_i, pix_f):
        ...
        # ### Save input scaler (scalers are the same)
